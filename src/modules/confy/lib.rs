...
use crate::modules::confy::utils::CheckedStringRead;
...
pub fn load_path<T: Serialize + DeserializeOwned + Default>(path: impl AsRef<Path>) -> Result<T, ConfyError> {
	match File::open(&path) {
		Ok(mut cfg) => {
			let cfg_string = cfg.get_string().map_err(ConfyError::ReadConfigurationFileError)?;
...
pub fn get_configuration_file_path<'a>(
	...
	let project = ProjectDirs::from("rs", "", app_name)
		.ok_or_else(|| ConfyError::BadConfigDirectory("could not determine home directory path".to_string()))?;
...
mod tests {
	...
	fn test_store_path_root_error() {
		let err = store_path(PathBuf::from("/"), &ExampleConfig::default()).expect_err("store_path should fail");
		assert_eq!(
			err.to_string(),
